...
- **Do NOT run `pnpm run check`** — it chains typecheck + lint + fmt + the full suite + web:typecheck and OOMs this ~3.8GB box. **`pnpm test` IS safe**: `package.json:16` maps it to `node scripts/test-all.mjs`, which exists precisely to run the workers-pool projects SEQUENTIALLY. (Corrected 2026-09-02 — the earlier constraint conflated the two, and Task 3's fix round caught it.) Six vitest projects: `unit`, `web-unit`, `integration-fast`, `integration-grouped`, `integration-shared`, `integration-isolated`. While iterating, run only the projects whose globs cover your changed files; mutation proofs run the SINGLE test file. Run all six ONCE, as a single invocation, immediately before your commit.
