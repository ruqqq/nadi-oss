...
/** Duration label from server-persisted timing: live for a running run, frozen
 *  at finish for a terminal run. Server `startedAt` survives a page refresh. */
export function runDurationLabel(opts: {
  startedAt?: number;
  finishedAt?: number;
  nowMs: number;
}): string | undefined {
  if (opts.startedAt === undefined) return undefined;
  const end = opts.finishedAt ?? opts.nowMs;
  return formatElapsed(Math.max(0, end - opts.startedAt));
}

/**
 * Effective timing for the duration label. Prefers the server's `finishedAt`
 * (survives a page refresh), but falls back to a client-observed terminal
 * timestamp so a finished run's timer freezes even when the server hasn't yet
 * persisted `finishedAt` (or never will, e.g. an aborted run). Without this the
 * label would fall back to the live `nowMs` and keep climbing after the run is
 * already `completed`/`error`/`aborted`.
 */
export function effectiveRunTiming(
  server: { startedAt?: number; finishedAt?: number } | undefined,
  clientTerminalAt: number | undefined,
): { startedAt?: number; finishedAt?: number } {
  const finishedAt = server?.finishedAt ?? clientTerminalAt;
  return {
...
export function subagentCardModel(
  run: SubagentRunView,
  opts: { firstSeenMs?: number; nowMs: number },
): SubagentCardModel {
  const progressLine = run.progress?.message ?? run.progress?.phase;
  const elapsedLabel =
    opts.firstSeenMs !== undefined ? formatElapsed(opts.nowMs - opts.firstSeenMs) : undefined;
  return {
    title: subagentCardTitle(run),
    statusLabel: statusLabelFor(run),
    tone: subagentTone(run.status),
