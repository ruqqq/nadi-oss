  ...
  return (
    <PromptInputHoverCard>
      <HoverCardTrigger asChild>
        <div
          className={cn(
            "group relative flex h-8 min-w-0 max-w-48 shrink-0 cursor-pointer select-none items-center gap-1.5 rounded-md border border-border px-1.5 font-medium text-sm transition-all hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50",
            className,
          )}
          key={data.id}
          {...props}
...
export type PromptInputProps = Omit<HTMLAttributes<HTMLFormElement>, "onSubmit" | "onError"> & {
  ...
  // Non-blocking: called with added files the current model can't read inline.
  onModelUnsupported?: (files: File[]) => void;
  // Predicate: whether the current model can read a file as native content.
  isSupportedForModel?: (file: File) => boolean;
};
...
export const PromptInput = ({
  ...
  const matchesAccept = useCallback(
    (f: File) => fileMatchesAccept({ type: f.type, name: f.name }, accept ?? ""),
    [accept],
  );
  ...
  const addLocal = useCallback(
    (fileList: File[] | FileList) => {
      ...
      if (onModelUnsupported && isSupportedForModel) {
        const unsupported = sized.filter((f) => !isSupportedForModel(f));
        if (unsupported.length > 0) onModelUnsupported(unsupported);
      }
    ...
    [matchesAccept, maxFiles, maxFileSize, onError, onModelUnsupported, isSupportedForModel],
  );
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = (event) => {
    ...
    const clearSubmittedTextIfUnchanged = () => {
      if (usingProvider) {
    ...
    // Convert blob URLs to data URLs asynchronously, or delegate to uploadAttachments
    const fileParts = files.map(({ id: _id, ...item }) => item);
    const filesPromise = uploadAttachments
      ? uploadAttachments(fileParts)
      : Promise.all(
